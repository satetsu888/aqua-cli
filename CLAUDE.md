...
- **Soft Quota**: 実行前に `GET /api/quota/status` でクォータ状態をプリチェックし、超過時はサーバーへのデータ記録を全スキップしてテストをローカル実行（警告表示のみ）。テスト自体は常に実行可能。プリチェック失敗時はフォールバックで通常記録を試み、サーバーの 402 がセーフティネットとなる
