...
- サーバーがクォータ超過時に HTTP 402 Payment Required を返す場合がある（クラウド版のみ）。CLI はエラーメッセージをそのまま表示
...
- `execute_qa_plan` - QA Plan を実行（全シナリオを順次実行し結果を記録。`env_name` で環境選択可能。レスポンスにはマスク済み resolved variables と Web UI の実行結果 URL を含む）
